...
interface ChatWindowProps extends OriginalChatWindowProps {
...
const MessageItem: React.FC<MessageItemProps> = memo(({ virtualRow, message, isDarkTheme, selectedChatId, onRetryDecryption, measureRef }) => {
  ...
  const renderContent = () => {
    if (msg.type === 'image') {
      return (
        <img
          src={`https://100.64.221.88:4000${msg.text}`}
          alt="Sent image"
          style={{ maxWidth: '100%', borderRadius: '8px', cursor: 'pointer' }}
          onClick={() => window.open(`https://100.64.221.88:4000${msg.text}`, '_blank')}
        />
      );
    } else if (msg.type === 'file') {
      return (
        <a
          href={`https://100.64.221.88:4000${msg.text}`}
          target="_blank"
          rel="noopener noreferrer"
          style={{ color: isDarkTheme ? '#00C7D4' : '#007bff', textDecoration: 'underline' }}
        >
          {msg.text.split('/').pop() || 'Download file'}
        </a>
      );
    }
    return (
      <span style={{ marginRight: msg.text.startsWith('base64:') ? '15px' : '0' }}>
        {msg.text}
      </span>
    );
  };
...
const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    initialOffset: messages.length > 0 ? (messages.length - 1) * estimateSize() : 0,
  });
      ...
      <div ref={inputAreaRef} className="message-input-container">
        <div className="input-inner-container">
          ...
          <input
            ...
            accept="image/*,.pdf,.doc,.docx,.txt" // Обмеження типів файлів
          />
          <input
            type="text"
